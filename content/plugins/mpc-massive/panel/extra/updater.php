...
class MPC_Plugin_Updater {
	...
	private function init_plugin_data() {
		if( empty( $this->plugin_data ) ) {
			...
			$this->update_urls = array(
				'main'      => 'https://mpcreation.net/api/updates/' . $this->short_slug . '/update.json',
				'secondary' => 'http://mpcthemes.net/api/updates/' . $this->short_slug . '/update.json',
			);
	...
	public function get_update_info() {
		...
		$download_link = 'main';
		$request = wp_remote_post( $this->update_urls[ 'main' ], $http_args );

		if ( is_wp_error( $request ) ) {
			$download_link = 'secondary';
			$request = wp_remote_post( $this->update_urls[ 'secondary' ], $http_args );
		}
	...
	public function set_transient( $transient ) {
		...
		if( isset( $this->plugin_update->version ) && isset( $this->slug ) && isset( $transient->checked[ $this->slug ] ) ) {
			$do_update = version_compare( $this->plugin_update->version, $transient->checked[ $this->slug ] );
		} else {
			return $transient;
		}
		...
		if ( $do_update === 1 ) {
			...
			$this->plugin_update->version     = $transient->checked[ $this->slug ];
