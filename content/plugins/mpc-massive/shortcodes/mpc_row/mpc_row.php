	...
	class MPC_Row {
		...
		function __construct() {
			...
			if ( isset( $mpc_ma_options[ 'vc_row_addons' ] ) && $mpc_ma_options[ 'vc_row_addons' ] == '1' ) {
				...
				add_filter( 'vc_shortcode_output', array( $this, 'append_class' ), 1000, 3 );
			}
		}
		...
		public function row_output( $output, $shortcode, $atts ) {
			...
			$content_html = new DOMDocument( '1.1' );
			...
			if( $mpc_can_link && $this->atts[ 'url' ] != '' && stripos( $output, '<a' ) !== false ) {
				if( current_user_can( 'edit_posts') ) {
					$this->before .= '<div class="mpc-notice mpc-cannot-link">' . __( '<strong>Massive Addons</strong>: Link Block could not be added to this row because it contains a link already. Check our documentation for more information: <a href="http://hub.mpcreation.net/knowledgebase/link-column/">Link Block.</a> <br/>Psss.. This information is not visible for your visitors.', 'mpc' ) . '</div>';
				}
				$mpc_can_link = false;
			}
			...
			// Prepend Row
			if( $this->prepend != '' ) {
				$content_html->loadHTML( $this->prepend );
			...
			// Append Row
			if( $this->append != '' ) {
				$content_html->loadHTML( $this->append );
				$nodes = $content_html->documentElement->firstChild->childNodes;
		...
		function row_class() {
			...
			if ( $this->atts[ 'enable_bottom_separator' ] == 'true' || $this->atts[ 'enable_bottom_separator' ] == 'true' ) {
