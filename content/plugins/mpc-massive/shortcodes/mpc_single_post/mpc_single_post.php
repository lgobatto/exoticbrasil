...
if ( ! class_exists( 'MPC_Single_Post' ) ) {
	class MPC_Single_Post {
		...
		function get_placeholder( $thumbnail_layout ) {
			$placeholder = '<div class="mpc-post__thumbnail mpc-image-placeholder">' . $thumbnail_layout . '</div>';
		...
		/* Build Thumbnail */
		function build_thumbnail( $layout, $disable, $tiles_size, $atts ) {
			if ( $disable == '' ) {
				...
				if( $thumbnail !== false ) {
					if( $tiles_size !== 'full' ) {
						...
						$thumbnail = 'style="background-image: url('. $thumbnail . ');' . $per_thumb_css . '"';
						$return = '<div class="mpc-post__thumbnail"' . $thumbnail . '>' . $thumbnail_layout . '</div>';
					} else {
		...
		function pagination_content( $atts ) {
			...
			$this->parts[ 'vertical_begin' ] = '<div class="mpc-post--vertical-wrap"><div class="mpc-post--vertical">';
			...
			$this->parts[ 'thumbnail' ] = $this->build_thumbnail( $atts[ 'layout' ], $atts[ 'disable_thumbnail' ], $atts[ 'tiles_size' ], $atts );
