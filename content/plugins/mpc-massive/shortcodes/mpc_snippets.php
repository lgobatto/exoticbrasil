...
if( !class_exists( 'MPC_Helper' ) ) {
	class MPC_Helper {
		...
		static function search_scripts( &$output, $offset = 0 ) {
			...
			if( $match = preg_replace( "/<script([\S|.|\s]*)<\/script>/mU", '<mpc_' . $script_uid . '" />', $output, 1 ) ) {
				$output = $match;
			}
		...
		static function search_styles( &$output, $offset = 0 ) {
			$start_at = stripos( $output, '<style', $offset );
			if( $start_at === false ) { return false; }

			$ends_at = stripos( $output, '</style>', $start_at ) + 8; // 9 = strlen( '</style>' )
			if( $ends_at === false && $start_at == $ends_at ) { return false; }
			...
			if( $match = preg_replace( "/<style([\S|.|\s]*)<\/style>/mU", '<mpc_' . $style_uid . '" />', $output, 1 ) ) {
				$output = $match;
			}
		...
		static function post_parse_scripts( &$output ) {
			...
			foreach( $scripts as $script_uid => $replacement ) {
				if( $match = preg_replace( '/<mpc_' . $script_uid . '><\/mpc_' . $script_uid . '>/', $replacement, $output ) ) {
					$output = $match;
				}
			}
	...
	class MPC_Autocompleter {
		...
		static function render_wc_product( $query ) {
			...
			if ( ! empty( $query ) ) {
				...
				$variation_object = ( $product_object->product_type === 'variation' ) ? new WC_Product_Variation( (int) $query ) : null;

				if ( is_object( $variation_object ) && $variation_object->parent !== null ) {
					$product_sku   = $variation_object->get_sku();
					$product_title = $variation_object->get_title();
					$variation_id  = $product_object->variation_id;
