...
if ( ! class_exists( 'MPC_WC_Product' ) ) {
	class MPC_WC_Product {
		...
		/* Build Thumbnail */
		function build_thumbnail( $tiles_size, $atts ) {
			...
			if( $tiles_size !== 'full'  ) {
				...
				$return .= '<div class="mpc-product__thumb mpc-effect--target' . $is_placeholder . '" ' . ( $is_placeholder == '' ? $thumbnail : '' ) . '></div>';
			} else {
				$thumbnail_css = 'style="background-image: url('. $thumbnail . ');"';
				$return = '<div class="mpc-product__thumb-wrap">';
				$return .= '<div class="mpc-product__thumb mpc-effect--target"' . $thumbnail_css . '></div>';
		...
		function pagination_content( $atts ) {
			...
			$this->parts[ 'thumb_hover_content' ] = $atts[ 'thumb_hover_elements' ] != '' ? '<div class="mpc-thumb__content-hover mpc-transition">' . $this->build_from_list( $atts[ 'thumb_hover_elements' ], '', false ). '</div>' : '';
			$this->parts[ 'thumbnail' ] = $this->build_thumbnail( $atts[ 'tiles_size' ], $atts );
