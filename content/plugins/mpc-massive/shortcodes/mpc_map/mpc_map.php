		...
		function shortcode_map() {
			...
			$base = array(
				...
				array(
					...
					'param_name'       => 'location',
					'admin_label'      => true,
					'tooltip'          => __( 'Define location for the map center point. Please provide the full address (e.g. <em>Mountain View, CA, USA</em>).', 'mpc' ),
					'value'            => '',
					'dependency'  => array(
...
//				'icon'            => mpc_get_plugin_path( __FILE__ ) . '/assets/images/icons/mpc-google-maps.png',
		...
		function get_map_location( $location ) {
			...
			if ( is_array( $mpc_map_locations ) && isset( $mpc_map_locations[ $location_md5 ] ) ) {
				...
				$response = wp_remote_get( 'http://maps.googleapis.com/maps/api/geocode/json?address=' . urlencode( $location ) . '&sensor=false' );
